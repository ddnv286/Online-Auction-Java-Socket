...
public class CustomerThread implements Runnable, Serializable {
	...
	public CustomerThread(ObjectInputStream input, ObjectOutputStream output, String name, String password, int ID)
			throws Exception {
		this.input = input;
		this.output = output;
		this.name = name;
	...
	public void run() {
		try {
			CustomerInfo customer = Jdbc.LoadCustomerState(name, ID);
