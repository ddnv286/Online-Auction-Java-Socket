...
public class TimerThread implements Runnable, Comparator<PriorityBid> {
	...
	public TimerThread(ObjectInputStream input, ObjectOutputStream output, Start parent, Sell sell,
			AuctionRunning parent2, StartAuction startAuction) {
		this.parent = parent;
		this.input = input;
		this.output = output;
		this.sell = sell;
		...
		try {
			synchronized (output) {
				...
				while (i.hasNext()) {
					item = i.next();
					Barray = item.b;
					File file = new File("F:\\Software\\eclipse\\projects\\Online Auction\\Admin\\AdminImages\\"+ String.valueOf(item.ID) + ".txt");
					fos = new FileOutputStream(file);
					fos.write(Barray);
					image = new ImageIcon("F:\\Software\\eclipse\\projects\\Online Auction\\Admin\\AdminImages\\"+ String.valueOf(item.ID) + ".txt");
