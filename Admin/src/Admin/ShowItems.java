...
public class ShowItems extends JPanel {
	...
	public ShowItems(ObjectInputStream input, ObjectOutputStream output) throws Exception {
		...
		while (i.hasNext()) {
			item = i.next();
			Barray = item.b;
			File file = new File("F:\\Software\\eclipse\\projects\\Online Auction\\Admin\\AdminImages\\"
					+ String.valueOf(item.ID) + ".txt");
			fos = new FileOutputStream(file);
			fos.write(Barray);
			image = new ImageIcon("F:\\Software\\eclipse\\projects\\Online Auction\\Admin\\AdminImages\\"
					+ String.valueOf(item.ID) + ".txt");
